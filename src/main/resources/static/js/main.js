...
layui.use(['layer', 'element'], function() {
    ...
    element.on('tab(admin-tab)', function(){
        ...
        var dd=$(parent.document).find("a[lay-id='"+layid+"']").parent();
        var odd=$(parent.document).find("a:not([lay-id='\"+layid+\"'])").parent();
        odd.removeClass("layui-this")
    ...
    active = {
        ...
        tabDelete2: function(obj){
            var lay_id = $(this).attr("lay-id");
            element.tabDelete("admin-tab", lay_id);
            var title = $(this).html() + '<i class="layui-icon" data-id="' + lay_id + '"></i>';
            //新增一个Tab项
            var content = '<iframe src="' + $(this).attr('data-url') + '"></iframe>'
            element.tabAdd('admin-tab', {
                title: title,
                content: content,
                id: lay_id
    ...
    $(document).on('click','a',function(){
        ...
        /*if(lay_id=="43"){
            active["tabAdd2"].call(this);
            resize();
            return;
        }*/
        for(var i = 0; i < tabs.length; i++) {

            if($(tabs).eq(i).attr("lay-id") == lay_id /*&& $(tabs).eq(i).attr("class") != "layui-this"*/) {
                active["tabDelete2"].call(this);
                resize();
                //active.tabChange(lay_id);
                return;
            }

            /*else if ($(tabs).eq(i).attr("lay-id") == lay_id && $(tabs).eq(i).attr("class") == "layui-this") {
                 active["tabDelete"].call(lay_id);
